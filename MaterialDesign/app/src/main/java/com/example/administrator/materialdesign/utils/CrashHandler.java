...
import com.example.administrator.materialdesign.utils.logUtils.SaveLogs2FileUtil;
...
public class CrashHandler implements Thread.UncaughtExceptionHandler {
    ...
    /**
     * {@inheritDoc}
     * <p>当UncaughtException发生时会转入该函数来处理 </p>
     */
    @Override
    ...
    private void restartApp(final Throwable ex) {
        new Thread() {
            @Override
            public void run() {
                ...
                mgr.set(AlarmManager.ELAPSED_REALTIME_WAKEUP, System.currentTimeMillis() + 3000, restartIntent);
     ...
     * @param ctx
     */
    public synchronized void collectDeviceInfo(Context ctx) {
        try {
    ...
    private synchronized  String saveCrashInfo2File(Throwable ex) {
        ...
        sb.append(dateStr);
        sb.append("\r\n"+result);
        SaveLogs2FileUtil.uploadLogs(sb.toString());
        Log.e("gt",sb.toString());
        return null;
    }
}